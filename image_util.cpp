...
RGBQUAD** getRGB(BITMAPINFOHEADER fileInfoHeader){
    ...
    // чтение
    unsigned int bufer;
    std::ifstream fileStream(fileInfoHeader.filename, std::ifstream::binary);
    for (unsigned int i = 0; i < fileInfoHeader.biHeight; i++) {
...
BITMAPINFOHEADER getImageInfo(char* fileName){
    ...
    fileInfoHeader.filename = fileName;
    ...
    if (fileInfoHeader.biBitCount != 16 && fileInfoHeader.biBitCount != 24 && fileInfoHeader.biBitCount != 32) {
        std::cerr << "Error: Unsupported BMP bit count." << std::endl;
        return fileInfoHeader;
    }

    if (fileInfoHeader.biCompression != 0 && fileInfoHeader.biCompression != 3) {
        std::cerr << "Error: Unsupported BMP compression." << std::endl;
        return fileInfoHeader;
    }
    return fileInfoHeader;
}
